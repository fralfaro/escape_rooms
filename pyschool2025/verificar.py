...
def salon_33(respuesta):
    if respuesta is None:
        text("Debes definir la variable `respuesta` como una lista con 4 elementos en el orden [HP, Attack, Defense, Speed].", "info")
    elif respuesta ==  ['Dragon', 'Dragon', 'Steel', 'Flying']:
        hyperlink("¡Correcto! Identificaste los tipos con mayor promedio en [HP, Attack, Defense y Speed]. Avanza a la siguiente sala.", "4.html", "success")
    else:
        text("Revisa bien los datos... ¿Estás seguro de haber calculado los promedios correctamente?", "warning")

def salon_44( respuesta):
    if respuesta is None:
        text("Debes definir la variable `respuesta` el tipo de correlación entre las estadísticas seleccionadas.", "info")
    elif respuesta == 'positiva' :
        hyperlink("¡Correcto! Identificaste correctamente el tipo de correlación entre las estadísticas seleccionadas. Avanza a la siguiente sala.", "end.html", "success")
    else:
        text("Revisa bien los datos... ¿Estás seguro de haber calculado el tipo de correlación correctamente?", "warning")
